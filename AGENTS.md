...
Topic consultations use **Qdrant** (transcript chunk vectors) and **OpenAI** (query embedding + grounded answers). Vectors are **not** in Postgres; an external embed worker acks via `/api/content/embedding-ingest/`.
...
- **Seed data** (`populate_content`) sets `embedding_status=skipped`, so Consultas will not appear until transcripts are acked as `indexed` (manually in dev or via the embed worker).
