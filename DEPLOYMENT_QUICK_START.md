...
- **Email:** Leave `SEND_EMAILS` unset (or `false`) until the SMTP2GO domain is verified; then set `SEND_EMAILS=true` and the `EMAIL_HOST*` / `EMAIL_HOST_USER` / `EMAIL_HOST_PASSWORD` vars in `acbc_app/.env`. See [Email features](docs/deployment/email-features.md).
